...
| Field | Use |
|-------|-----|
| `ActivePage` | Sidebar highlight (`"home"`, `"doctrees"`, `"publish"`, `"apikeys"`, `"mcp"`, …) |
...
**Access pages:** `/_monms/doctrees` (`doctrees.go`, all authenticated — markdown tree browser, **alignment audit** banner, publisher migration panel), `/_monms/api-keys` (`apikeys.go`, gate `requireCanManageAPIKeys`), `/_monms/mcp` (`mcp_settings.go`, `requireSuperuser`). Engine collections: `internal/authbootstrap` (`users`, `monms_api_keys`), keys logic `internal/apikeys`, MCP server `internal/mcp`. Document trees: `internal/documents/tree.go` (`BuildForest`).
