...
MonMS enables external AI agents to safely mutate a Git-tracked site during **shaping** — the consultant/operator phase where site design (templates, schema, assets) is defined. Agents can:
...
**Structure rail only (shaping):** Agents and consultants mutate **L2 structure** (templates, schema, assets). Editorial **content** (L3) is client-driven on the **staging** instance via `/_monms/publish` — agents do not routine-push copy to production. Content lives in `.pb_data/`, not Git. See [README.md](README.md) § content publish and [MEDIA.md](MEDIA.md).

**Phase 2 safety guardrails** protect the site from invalid mutations:
...
3. **Pre-commit hook with automatic rollback** — on any validation failure, `git checkout -- .` restores the site and the commit is aborted (D-43, AGT-05).
...
2. Add `monms` to `$PATH`:
   ...
   ```
3. Place the binary at `../../monms` relative to the site directory.
...
Content CLI (operator fallback): `monms content export|import|diff|publish --site .` — see [README.md](README.md).

### SSH Key

Use a dedicated SSH key restricted to the site subdirectory only. See `SECURITY.md` §SSH Key Scope.


## Schema Dual-Write Workflow (D-33, AGT-01)

When creating a new collection, always perform a **dual-write**: create the collection live via the API, then persist the schema JSON to the site for audit and bootstrap sync.
...
### Step 2 — Write matching JSON to site/schema/

```bash
cat > site/schema/press_releases.json << 'EOF'
{
  "name": "press_releases",
  "type": "base",
  "fields": [
...
Template changes are visible on the next HTTP request without restarting the server. In production builds, fsnotify watches the **entire site tree** for `.gohtml` changes and flushes the in-memory cache (D-30, AGT-02).
...
The pre-commit hook installed at `site/.git/hooks/pre-commit` runs automatically on every `git commit`. It:
...
3. Runs `monms validate --site .` against staged files only.
...
```bash
MONMS_BIN=/path/to/monms monms validate --site .
```

Or with explicit file paths:

```bash
monms validate --site . --files templates/press/index.gohtml
```
